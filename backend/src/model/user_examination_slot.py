...
from src.model.enum import PaymentStatus, UserExaminationStatus
...
class UserExaminationSlotClass(AuditTrailMixin, base):
    ...
    examination_slot_id = Column(
        UUID(as_uuid=True),
        ForeignKey("ExaminationSlotTable.examination_slot_id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    user_id = Column(
    ...
    user_examination_status = Column(
        SQLAlchemyEnum(UserExaminationStatus),
        default=UserExaminationStatus.PENDING,
        nullable=False,
    )
    ...
    user = relationship(
        "UserClass", 
        back_populates="examination_slots"
        )
    examination_slot = relationship(
